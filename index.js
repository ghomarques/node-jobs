    ...
    try {
        ...
        const create = () => {
            createJobs(jobs, slackConfig)
                .then(instances => {
                    logger.debug('created jobs', instances.map(instance => instance.name));
                    // run specific job
                    if (hasSpecificJob) {
                        instances.forEach(instance => {
                            job.forEach(specificJob => {
                                if (instance.name === specificJob) {
                                    if (once) { // if once, execute now independent of job autoStart, will not start cronJob
                                        instance.exec()
                                            .then(result => {
                                                logger.debug('success on job execution', specificJob, result);
                                            })
                                            .catch(error => {
                                                logger.error('error on job execution', specificJob, error);
                                            });
                                    } else { // if !once, start cronJob, independent of job autoStart, will start cronJob
                                        instance.start();
                                    }
                                }
                            });
